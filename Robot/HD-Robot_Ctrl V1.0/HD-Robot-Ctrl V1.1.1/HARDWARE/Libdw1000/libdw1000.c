...
/*默认操作模式*/
const uint8_t MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024};
//const uint8_t MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_128};
...
void dwInit(dwDevice_t* dev, dwOps_t* ops)
{
    ...
    writeValueToBytes(dev->antennaDelay.raw, 16384, LEN_STAMP);
	  //writeValueToBytes(dev->antennaDelay.raw, 0, LEN_STAMP);
}
...
/*
*******************************************************************************
*	函 数 名: dwEnableAllLeds
*	功能说明: 对DW1000收发LED指示控制; 外部放大器控制
*	形	  参: 无
*	返 回 值: 无
*******************************************************************************
*/
...
{
    setBit(dev->sysmask, LEN_SYS_MASK, LDEERR_BIT, val);
    setBit(dev->sysmask, LEN_SYS_MASK, RXFCE_BIT, val);
    setBit(dev->sysmask, LEN_SYS_MASK, RXPHE_BIT, val);
    setBit(dev->sysmask, LEN_SYS_MASK, RXRFSL_BIT, val);
...
{
    setBit(dev->sysctrl, LEN_SYS_CTRL, SFCST_BIT, !dev->frameCheck);
    setBit(dev->sysctrl, LEN_SYS_CTRL, RXENAB_BIT, false);
    dwSpiWrite(dev, SYS_CTRL, NO_SUB, dev->sysctrl, LEN_SYS_CTRL);
}
...
void dwSetDefaults(dwDevice_t* dev) 
{
    ...
    //for global frame filtering
		...
		dwEnableMode(dev, MODE_LONGDATA_FAST_ACCURACY);
	}
}
...
void dwTune(dwDevice_t *dev) 
{
	...
	// DRX_TUNE1b
	if(dev->preambleLength ==  TX_PREAMBLE_LEN_1024  || dev->preambleLength ==  TX_PREAMBLE_LEN_1536 || dev->preambleLength ==  TX_PREAMBLE_LEN_2048 ||
...
*	函 数 名: setBit
*	功能说明: 寄存器置位函数
*	形	  参: data[]:寄存器数组，bit:未选择，val:位设置
*	返 回 值: 无
*******************************************************************************
*/
static void setBit(uint8_t data[], unsigned int n, unsigned int bit, bool val)
