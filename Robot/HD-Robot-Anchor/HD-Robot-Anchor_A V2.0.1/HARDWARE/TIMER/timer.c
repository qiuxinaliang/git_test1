...
*	功能说明: 初始化独立看门狗
*	形	  参: psc：时钟预分频数
*	返 回 值: 无(定时器溢出时间计算方法:Tout=((arr+1)*(psc+1))/Ft us.)
*******************************************************************************
*/
...
void MX_TIM6_Init(void)
{
  ...
  htim6.Init.Period = 250 -1;  // 计数值为500，2500 / 250 = 10 表示每1s进10次中断
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *htim)
{
	...
	HAL_NVIC_SetPriority(TIM3_IRQn,2,0);    //设置中断优先级，抢占优先级1，子优先级3
